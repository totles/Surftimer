...
char sql_selectFastestBonus[] = "SELECT b.name, b.runtime, b.zonegroup, b.style FROM (SELECT MIN(runtime) as runtime, style FROM ck_bonus WHERE mapname='%s' GROUP BY style) a JOIN ck_bonus b using(style,runtime);";
...
char sql_selectMapRecord[] = "SELECT b.runtimepro,b.name,b.steamid,b.style FROM (SELECT style,min(runtimepro) AS runtimepro FROM ck_playertimes WHERE mapname='%s' GROUP BY style) a JOIN ck_playertimes b using(style,runtimepro);";
