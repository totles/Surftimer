...
public void ZoneMenu(int client)
{
	...
	if (IsPlayerZoner(client))
	{
		...
		ckZoneMenu.SetTitle("设置区域页面");
		ckZoneMenu.AddItem("", "创建一个区域");
...
public void EditZoneGroup(int client)
{
	...
	editZoneGroupMenu.SetTitle("你想要编辑哪个区域?");
	editZoneGroupMenu.AddItem("1", "主线区域");
	editZoneGroupMenu.AddItem("2", "奖励关区域");
	editZoneGroupMenu.AddItem("3", "功能区域");
...
public void ListBonusGroups(int client)
{
	...
	h_bonusGroupListing.SetTitle("可用的奖励关");
...
public void ListBonusSettings(int client)
{
	...
	h_ListBonusSettings.SetTitle("为 %s 设置", g_szZoneGroupName[g_CurrentSelectedZoneGroup[client]]);

	h_ListBonusSettings.AddItem("1", "创建新区域");
	h_ListBonusSettings.AddItem("2", "列出奖励关所有区域");
	h_ListBonusSettings.AddItem("3", "重新命名");
	h_ListBonusSettings.AddItem("4", "删除奖励关(包括记录)");
...
public void checkForMissclick(int client)
{
	...
	h_checkForMissclick.SetTitle("确定要删除 %s 的所有区域(包括记录?", g_szZoneGroupName[g_CurrentSelectedZoneGroup[client]]);

	h_checkForMissclick.AddItem("1", "否");
	h_checkForMissclick.AddItem("2", "否");
	h_checkForMissclick.AddItem("3", "是的");
...
public void SelectBonusZoneType(int client)
{
	...
	h_selectBonusZoneType.SetTitle("选择奖励关区域类型");

	h_selectBonusZoneType.AddItem("1", "起点");
...
public void SelectZoneGroup(int client)
{
	...
	newZoneGroupMenu.SetTitle("你想创建什么区域?");

	newZoneGroupMenu.AddItem("1", "主线区域");
	newZoneGroupMenu.AddItem("2", "奖励关区域");
	newZoneGroupMenu.AddItem("3", "功能区域");
...
public void StartBonusZoneCreation(int client)
{
	...
	CreateBonusFirst.SetTitle("创建奖励关起点区域:");
	if (g_Editing[client] == 0)
		CreateBonusFirst.AddItem("1", "开始画线");
	else
	{
		CreateBonusFirst.AddItem("1", "重新画线");
		CreateBonusFirst.AddItem("2", "保存奖励关起点区域");
...
public void EndBonusZoneCreation(int client)
{
	...
	CreateBonusSecond.SetTitle("创建奖励关终点区域:");
	if (g_Editing[client] == 2)
		CreateBonusSecond.AddItem("1", "开始画线");
	else
	{
		CreateBonusSecond.AddItem("1", "重新画线");
		CreateBonusSecond.AddItem("2", "保存奖励关终点区域");
	}
...
public void SelectNormalZoneType(int client)
{
	...
	SelectNormalZoneMenu.AddItem("hook", "可借用区域");
...
public void ZoneSettings(int client)
{
	...
	ZoneSettingMenu.SetTitle("通用区域设置");
	switch (GetConVarInt(g_hZoneDisplayType))
	{
		case 0:
			ZoneSettingMenu.AddItem("1", "线条可视设置：不可视");
		case 1:
			ZoneSettingMenu.AddItem("1", "线条可视设置：显示一半");
		case 2:
			ZoneSettingMenu.AddItem("1", "线条可视设置：全部显示");
	}

	switch (GetConVarInt(g_hZonesToDisplay))
	{
		case 1:
			ZoneSettingMenu.AddItem("2", "线条区域显示设置: 起点 & 终点");
		case 2:
			ZoneSettingMenu.AddItem("2", "线条区域显示设置: 起点, 终点, 关卡,奖励关");
		case 3:
			ZoneSettingMenu.AddItem("2", "线条区域显示设置: 所有区域");
	}
...
public void SelectMiscZoneType(int client)
{
	...
	SelectZoneMenu.SetTitle("选择功能区域类型");
	...
	SelectZoneMenu.AddItem("9", "防跳");
	SelectZoneMenu.AddItem("10", "防蹲");
	SelectZoneMenu.AddItem("11", "限速");
	SelectZoneMenu.AddItem("0", "停止计时器");
...
public void EditorMenu(int client)
{
	...
	if (g_ClientSelectedZone[client] != -1)
		editMenu.SetTitle("编辑区域中: %s-%i", g_szZoneDefaultNames[g_CurrentZoneType[client]], g_mapZones[g_ClientSelectedZone[client]][zoneTypeId]);
	else
		editMenu.SetTitle("创建一个新的 %s 区域中", g_szZoneDefaultNames[g_CurrentZoneType[client]]);

	// If creating a completely new zone, or editing an existing one
	if (g_Editing[client] == 0)
		editMenu.AddItem("", "开始画线");
	else
		editMenu.AddItem("", "重新画线");
	...
	if (g_Editing[client] > 0)
	{
		...
		switch (g_CurrentZoneTeam[client])
		{
			case 0:
			{
				editMenu.AddItem("", "将线设置为黄色");
			}
			case 1:
			{
				editMenu.AddItem("", "将线设置为绿色");
			}
			case 2:
			{
				editMenu.AddItem("", "将线设置为红色");
			}
			case 3:
			{
				editMenu.AddItem("", "将线设置为蓝色");
			}
		}
		editMenu.AddItem("", "前往区域");
		editMenu.AddItem("", "编辑区域范围");
		...
		if (g_ClientSelectedZone[client] != -1)
		{
			...
			if (g_mapZones[g_ClientSelectedZone[client]][oneJumpLimit] == 1)
				editMenu.AddItem("", "关闭一跳限制");
			else
				editMenu.AddItem("", "开启一跳限制");
...
public void ScaleMenu(int client)
{
	...
	char ScaleSize[128];
	Format(ScaleSize, sizeof(ScaleSize), "比例大小 %f", g_AvaliableScales[g_ClientSelectedScale[client]]);
...
public void PrespeedMenu(int client)
{
	...
	if ( g_mapZones[g_ClientSelectedZone[client]][preSpeed] == 0.0)
		Format(szTitle, sizeof(szTitle), "区域Prespeed (无限制)");
	else
		Format(szTitle, sizeof(szTitle), "区域Prespeed (%f)", g_mapZones[g_ClientSelectedZone[client]][preSpeed]);
	SetMenuTitle(menu, szTitle);

	AddMenuItem(menu, "-1.0", "自定义限制");
	AddMenuItem(menu, "-2.0", "取消限制");
...
public void SelectTrigger(int client, int index)
{
	...
	AddMenuItem(menu, szParam, "传送到该区域");
	AddMenuItem(menu, szParam, "借用区域");
...
public void ClearZonesMenu(int client)
{
	...
	hClearZonesMenu.SetTitle("你确定要删掉这图的所有区域?");
	hClearZonesMenu.AddItem("", "不要!");
	hClearZonesMenu.AddItem("", "不要!");
	hClearZonesMenu.AddItem("", "是的gkd!");
